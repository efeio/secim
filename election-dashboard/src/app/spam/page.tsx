...
export default function SpamPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setLoginError(d.error || "Şifre hatalı.");
      } else {
        setAuthenticated(true);
        loadPolls();
      }
  ...
  useEffect(() => {
    ...
    return () => {
      if (vpsIntervalRef.current) clearInterval(vpsIntervalRef.current);
    };
  }, []);
  ...
  if (!authenticated) {
    return (
      <div className="min-h-screen flex items-center justify-center px-4 animate-fade-in" style={{ background: "var(--bg-base)", color: "var(--text-primary)" }}>
        <div className="w-full max-w-md p-6 rounded-2xl glass border border-gray-800/40 space-y-6">
          <div className="text-center">
            <h2 className="text-xl font-bold">Spam Paneli</h2>
            <p className="text-xs mt-1 text-gray-400">Lütfen yönetici şifresini girin</p>
          </div>
          <form onSubmit={handleLogin} className="space-y-4">
            <input
              ...
              placeholder="Şifre"
              className="w-full bg-[#1a1a1f] border border-gray-700/60 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/50"
            />
            {loginError && <p className="text-xs text-red-400">{loginError}</p>}
            <button
              type="submit"
              disabled={loadingLogin}
              className="w-full py-3 rounded-xl bg-purple-600 active:scale-[0.98] font-bold text-sm transition-all disabled:opacity-50"
            >
              {loadingLogin ? "Doğrulanıyor..." : "Giriş Yap"}
            </button>
          </form>
        </div>
      </div>
          ...
          <div className="space-y-3 bg-[#111113] border border-gray-800/40 rounded-xl p-4">
            <div>
              ...
              <select
                ...
                className="w-full bg-[#1a1a1f] border border-gray-700/60 rounded-lg px-3 py-3 text-sm focus:outline-none"
              >
          ...
          <div className="bg-[#111113] border border-gray-800/40 rounded-xl p-4">
            ...
            <div className="flex flex-wrap gap-1.5">
              {recentProvinces.map((name, i) => (
                <span
                  key={`${name}-${i}`}
                  className="text-xs px-2 py-1 rounded-md bg-gray-800/60 text-gray-300 animate-fade-in"
